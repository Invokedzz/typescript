...
var repassword = document.querySelector('.password2');
...
function checkEmptyFields() {
    ...
    inputs.forEach(function (input) {
        if (!input.value)
            showErrorMessage(input, 'Insert valid information');
    });
}
function checkEmail(email) {
    if (!(0, isEmail_1.default)(email.value))
        showErrorMessage(email, 'Insert a valid email');
}
...
function showErrorMessage(input, msg) {
    ...
    var errorMSG = formField.querySelector('.error-message');
    errorMSG.innerText = msg;
    formField.classList.add(SHOW_ERROR_MESSAGES);
}
function sendForm(form) {
    var parameter = true;
    form.querySelectorAll('.' + SHOW_ERROR_MESSAGES).forEach(function () { return (parameter = false); });
    return parameter;
}
exports["default"] = 1;
